...
def update():#control all games introductions.
    ...
    if game_over == False:
        ...
        if fire_ball.colliderect(enemy) and attack == True:
...
def show_next_enemy_frame():
    ...
    print(f"Showing frame {death_index}")  # Debug
    
    if death_index < len(enemy_frames):
        enemy.image = enemy_frames[death_index]
        print(f"Changed to: {enemy_frames[death_index]}")  # Debug
        death_index += 1
        clock.schedule(show_next_enemy_frame, 0.5)
    else:
        print("Animation complete, regenerating enemy")  # Debug
